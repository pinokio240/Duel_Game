...
В ангаре можно сыграть в ЖРЕБИЙ: проклятья ослабляют ТОЛЬКО ВАС, но каждое
ДОБАВЛЯЕТ +15% очков; за них открываются облегчения, которые режут общий счёт.
Можно навесить баффы или дебаффы и НА ВРАГА — это тоже режет счёт. За матч
капают ОЧКИ, а лучшие 10 забегов попадают в ТАБЛИЦУ СЧЕТА (клавиша T).
Комбо «Лазер + Веер» стреляет ЛАЗЕРНЫМ ВЕЕРОМ — тремя лучами разом.
Десять арен, 10 бонусов (стены, мины, лазер, дым, ЭМИ), 5 раундов до победы.
...
Версия игры показана в главном меню внизу справа (сейчас v1.7).
...
- Каждое взятое проклятье открывает ещё одно облегчение (максимум 4 проклятья
  и 5 облегчений). Сняли проклятье — лишние облегчения снимаются сами.
...
## Эффекты на врага: баффы и дебаффы боту (B / N / M)

Можно навесить на бота и ослабления, и усиления — но **ЛЮБОЙ эффект на врага
режет общий счёт за забег**: дебафф −10%, бафф −5% за штуку (максимум 3
эффекта; облегчений они НЕ открывают — открывают только проклятья).
...
| Закалить врага | бафф | враг прочнее на 25% | −5% очков |
| Нитро врагу | бафф | враг быстрее на 15% | −5% очков |
| Ас за пультом | бафф | враг стреляет чаще | −5% очков |

Смелый план: взять 4 проклятья (+60% очков), 5 облегчений (−50%) и три
дебаффа врагу (−30%) — эквивалентный забег за x0.80, зато с каким танком!
...
каждое, облегчения режут на 10%, эффекты на врага — по своей цене.
...
— «НОВЫЙ РЕКОРД!». Таблица хранится в `duel_stats.json` и переживает
обновления игры.

...
очки (+10%..+15%)** — усиленный враг платит. Максимум 4 эффекта; облегчений
они НЕ открывают — открывают только проклятья. Баффы подсвечены зелёным,
дебаффы — рыжим.
...
Смелый план: взять 4 проклятья (+60%) и 4 баффа врагу (например, Берсерк
+ Нитро + Ас + Закалка = +45%) — забег за x2.05 против ОЗВЕРЕВШЕГО бота.
Трусишка, наоборот, мажет боту прицел и сыплет сырой порох — но счёт режется.
